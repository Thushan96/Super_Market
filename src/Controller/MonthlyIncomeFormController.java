...
public class MonthlyIncomeFormController {
    ...
    private void loadSecondData() throws SQLException, ClassNotFoundException {
        ResultSet rst = DbConnection.getInstance().getConnection().prepareStatement(" SELECT extract(MONTH FROM(OrderDate)) ,sum(OrderDetail.OrderQty)  FROM Orders INNER JOIN  OrderDetail ON Orders.OrderId = OrderDetail.OrderId GROUP BY OrderDate").executeQuery();
        while (rst.next()){
